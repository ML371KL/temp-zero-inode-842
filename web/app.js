...
(function () {
    ...
    return h('div', { 'class': 'hero' }, [
      ...
      v.rule ? h('div', { 'class': 'rule' }, [
        ...
        h('div', null, [
          ...
          // Правило стоит на ячейке ВОРОТ (publish.build_verdict), а статистика
          // выше — на режиме, и считается она по другим битам: у правила медиана
          // из замороженной таблицы по сырым признакам, у режима — живая по
          // гистерезису. Числа поэтому не совпадают, и без этой строки они
          // читаются как ошибка на одной карточке.
          regime && regime.label ? h('p', { 'class': 'rule__fine',
            text: 'Числа правила — из таблицы сочетаний по сырым признакам (раздел «Машина состояний»); статистика режима выше считается по воротам с гистерезисом, поэтому выборки разные.' }) : null
        ])
      ]) : null
    ]);
  }
  ...
  function renderStates(d) {
     ...
     * режим из трёх. Помечены ДВЕ строки, а не одна: «сейчас» — сырые признаки, на
     * которых стоит эта статистика, «ворота» — флаги с гистерезисом, по которым
     * стоит позиция. На месячной сетке они расходятся в 46 месяцах из 273, и одной
     * пометки хватало ровно до первого расхождения: 15.09.2026 таблица показала
     * «медведь · спокойно · стресс», а шапка — «медведь · стресс · стресс», и
     * читалось это как поломка. Оговорка в подписи была, но подпись мельче
     * противоречия. */
    var gateCell = (st.gate && st.gate.cell_code) || null;
      ...
      h('div', { 'class': 'tablewrap' }, [
        h('table', { 'class': 'data' }, [
          ...
          h('tbody', null, cellRows.map(function (c) {
            var isGate = !!gateCell && c.code === gateCell;
            var mark = c.current && isGate ? ' ◂ сейчас и ворота'
              : (c.current ? ' ◂ сейчас' : (isGate ? ' ◂ ворота' : ''));
            return h('tr', { 'class': (c.current || isGate) ? 'is-current' : null }, [
              h('th', { scope: 'row', text: cellWords(c.code) + mark }),
              h('td', { text: c.label || '—' }),
