...
(function () {
   ...
   *
   * Конвейер считает блок quotes каждые 15 минут именно ради витрины, но панель
  ...
  function renderQuotes(d) {
    ...
    var pills = keys.map(function (k) {
      var x = q[k];
      if (isNum(x.age_min)) age = Math.max(age == null ? 0 : age, x.age_min);
  ...
  function render(d, force) {
    ...
    // и нечего: витрина публикуется раз в 15 минут, а панель тянет её раз в минуту.
