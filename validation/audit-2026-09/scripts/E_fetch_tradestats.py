"""E_algopack шаг 2b: закачка 5-мин tradestats ALGOPACK по тяжеловесам индекса за 2020-01..2023-12
...
OUT = os.path.join(ROOT, "data", "algopack", "tradestats_raw")
...
KEEP = ["tradedate", "tradetime", "secid", "pr_open", "pr_high", "pr_low", "pr_close", "pr_std", "vol", "val",
        "trades", "pr_vwap", "trades_b", "trades_s", "val_b", "val_s", "vol_b", "vol_s", "disb", "pr_vwap_b", "pr_vwap_s"]
...
for t in tickers:
    for (yy, mm) in months:
        ...
        url = f"{L.ALGO}/datashop/algopack/eq/tradestats/{t}.json?from={frm}&till={till}&iss.meta=off"
