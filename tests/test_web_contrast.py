...
PAIRS = [
    ...
    # Акцент позиции — ТЕКСТ (слово «акции» в шапке, подсказка ленты), поэтому
    # порог текстовый на обеих поверхностях; заливка ленты позиции — графика.
    ("позиция «акции» — слово в шапке", "--accent", "--surface", TEXT_MIN),
    ("акцент в подсказке и легенде", "--accent", "--surface-2", TEXT_MIN),
    ("заливка «акции» на ленте позиции", "--accent", "--surface", GRAPHIC_MIN),
]
