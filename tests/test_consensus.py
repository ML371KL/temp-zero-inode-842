...
class TestSentenceScope(ConsensusCase):
    """Фильтры смотрят на ПРЕДЛОЖЕНИЕ со ставкой, а не на сообщение целиком.

    Оплачено заседанием 11.09.2026, когда автоматика не нашла ни одного верного
    кандидата и нашла один ложный:
      * опрос РБК 07.09 отброшен — прогноз стоял в первом предложении, а абзацем
        ниже РБК напоминал о июле: «регулятор снизил её на 25 б.п.»;
      * реплика Титова 10.09 принята за консенсус 10% — «консенсус» стоял в
        «Вашингтонскому консенсусу», а «до» совпало с началом слова «должна».
    Тексты ниже синтетические, но воспроизводят ровно ту структуру.
    """

    def test_историческая_справка_не_убивает_прогноз(self):
        text = ("Большинство участников консенсус-прогноза считают, что на заседании "
                "11 сентября ЦБ оставит ключевую ставку без изменений — на уровне 14%. "
                "Перед прошлым заседанием аналитики тоже ждали паузы, но регулятор "
                "снизил ставку на 25 б.п.")
        # мутация: проверять прошедшее время по всему тексту -> None.
        self.assertEqual(self.c._rate_from(text), 14.0)

    def test_вашингтонский_консенсус_не_опрос(self):
        text = ("Спикер считает, что политика идёт по Вашингтонскому консенсусу — "
                "повысим ставку, снизим инфляцию до 4%.")
        self.assertIsNone(self.c._rate_from(text))

    def test_до_не_совпадает_с_началом_слова_должна(self):
        # мутация: снять границы слов у глаголов -> 10.0, ошибка в 400 б.п.
        text = "Аналитики считают, что ключевая ставка должна быть ниже 10%"
        self.assertIsNone(self.c._rate_from(text))

    def test_слово_опроса_обязано_стоять_в_том_же_предложении(self):
        # Реплика Титова в миниатюре: «консенсус» — одно предложение, ставка — другое.
        text = ("Политика идёт по консенсусу для развивающихся стран. "
                "По словам спикера, ставку следует снизить до 10%.")
        self.assertIsNone(self.c._rate_from(text))

    def test_сценарий_меньшинства_через_бп_не_читается(self):
        """Шаг «на N б.п. — до X%» не разбирается НАМЕРЕННО.

        Перешагнуть точки «б.п.» пробовали 11.09.2026: опрос РБК к июньскому
        заседанию тут же стал читаться как 13,5% — миноритарный сценарий из той же
        статьи, при консенсусе 14,0. Пропуск сводки дешевле ошибки в 50 б.п.
        """
        text = ("Большинство участников консенсус-прогноза ждут шага в 50 базисных "
                "пунктов, но опрос аналитиков показал, что часть допускает пересмотр "
                "сразу на 100 б.п., до 13,5% годовых.")
        self.assertIsNone(self.c._rate_from(text))

    def test_сокращение_бп_не_рвёт_предложение_на_части(self):
        # Сплиттер не режет по точкам сокращения: иначе слово опроса и число
        # оказались бы в РАЗНЫХ предложениях и сводка потерялась бы по другой причине.
        parts = self.c._SENTENCE.split("Консенсус: снизит на 25 б.п. — до 13,75%. Далее")
        self.assertEqual(len(parts), 2)
        self.assertIn("13,75%", parts[0])

    def test_прогноз_во_втором_предложении_тоже_находится(self):
        text = ("Опубликован свежий обзор рынка. Консенсус-прогноз «Ведомостей»: "
                "ЦБ сохранит ставку на уровне 14,25% в июле.")
        self.assertEqual(self.c._rate_from(text), 14.25)
