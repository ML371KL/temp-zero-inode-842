...
Ключ R2 выпущен, конвейер публикует с VPS (интрадей каждые 5 мин, полный прогон 19:05 МСК),
...
Дальше панель обновляется сама: интрадей каждые 5 минут (только котировки), полный
...
- **Канал отказов**: `ERROR_BOT_TOKEN` и `ERROR_CHAT_ID` в том же env-файле — те же
  значения, что в `/usr/local/etc/dash-notify/env` (общий бот всех панелей). Туда и
  только туда уходят санитарные события; в журнал витрины они больше не попадают.
  Пусто = санитарные события не уходят никуда, кроме журнала прогона.
- **Комментарии к событиям**: `OPENROUTER_KEY` в том же env-файле, тот же ключ, что у
  837/838. Пусто = события уходят голым фактом. Модели только бесплатные; если основная
  перегружена, конвейер сам идёт по цепочке запасных (`pipeline/lib/commentary.py`).
  Разбор добавляет к прогону до ~2,5 мин, поэтому у цепочки есть общий бюджет времени
  (`LLM_BUDGET_S`, по умолчанию 240 с) — он не даёт медленному провайдеру съесть
  `RuntimeMaxSec` юнита и сорвать публикацию.
- **Лента хаба NEXUS**: `NEXUS_EVENTS_URL` (уже проставлен в шаблоне) и
  `NEXUS_INGEST_TOKEN` в том же env-файле. Токен **общий** с 837/838/840 — тот же, что
  лежит в секрете `NEXUS_INGEST_TOKEN` их репозиториев и в секретах воркера `nexus`;
  свой для 842 заводить нельзя, воркер сверяет одно значение на всех отправителей.
  Пустой токен = зеркала нет, конвейер этого не замечает. Для запасного писателя те же
  две переменные нужны в репозитории: секрет `NEXUS_INGEST_TOKEN` и переменная
  (Variables, не Secrets) `NEXUS_EVENTS_URL`.
