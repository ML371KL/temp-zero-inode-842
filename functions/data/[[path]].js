 ...
 * три живые панели мёртвыми.
 *
 * Одной этой функции для такого обещания НЕ ХВАТАЕТ, и это было проверено на проде:
 * пути с пустым сегментом (`/data//data.json`, `//data/data.json`) в маршрутизацию
 * `[[path]]` не попадают вовсе и уходят к статике за тем самым «200 и вёрстка».
 * Границу держит `functions/_middleware.js` — он пропускает сюда только канонические
 * пути, а остальным отвечает таким же JSON-404. Инвариант «под `/data/` ни один путь
 * не отвечает 200 с вёрсткой — ни на GET, ни на HEAD, ни на POST» живёт на двух файлах
 * сразу, и проверяется пробой деплоя в .github/workflows/site.yml.
 */
...
function jsonResponse(status, body, extraHeaders = {}) {
  return new Response(JSON.stringify(body), {
    ...
    headers: {
      ...
      // Заголовок ставится здесь, а не только в functions/_middleware.js, потому что
      // 404 этой функции — единственный ответ на всём сайте, который ОТРАЖАЕТ ввод
      // клиента (поле requested). Он и обязан нести запрет на угадывание типа сам, не
      // полагаясь на соседний файл: статике Pages ставит nosniff, функции — нет.
...
export async function onRequestGet({ env, request, params }) {
  ...
  // Тип объявлен явно — значит и угадывать его запрещаем явно. `writeHttpMetadata`
  // переносит из R2 только то, что положил писатель, а он про заголовки браузера
  // ничего не знает.
