...
# Дробления и консолидации акций корзины ширины. ISS отдаёт TQBR СЫРЫМИ ценами: в
# день дробления цена меняется в разы, и MA200 ещё двести торговых дней сравнивает
# рубли после дробления с рублями до него. Замер на сторе 10.08.2026: бит «выше
# MA200» залипал на одном значении 80–260 дней подряд у каждой из шести бумаг —
# 919 бумаго-дней, каждый из которых стоил ширине 1/45 = 2,2 п.п.
#
# Список ЯВНЫЙ, а не эвристика «скачок больше X»: при пороге 1,4x в неё попадают
# 24.02.2022 (сразу у всех бумаг), арест Евтушенкова (AFKS −37 % 17.09.2014), иск
# Роснефти (AFKS −37 % 03.05.2017) и реопен VKCO (+73 % 29.03.2022) — настоящие
# движения, «поправка» которых была бы порчей данных.
#
# Каждое событие подтверждено ОБОРОТОМ ISS в окне ±10 дней: при дроблении число
# бумаг в сделках меняется во столько же раз, во сколько цена, а оборот в рублях
# не меняется; настоящее движение цены объём не трогает (проверка — AFKS 18.12.2014:
# цена x1,79 при объёме x0,90, в список не попал).
# factor — во сколько раз умножить цены СТРОГО ДО даты, чтобы привести историю к
# масштабу после неё.
CORPORATE_ACTIONS = {
    "IRAO":  [("2015-01-20", 100.0)],   # консолидация 100:1; цена x103, объём /224, оборот x0,45
    "TRNFP": [("2024-02-21", 0.01)],    # дробление 1:100; цена /99,2, объём x98,9, оборот x1,00
    "GMKN":  [("2024-04-08", 0.01)],    # дробление 1:100; цена /94,0, объём x162,6, оборот x1,71
    "VTBR":  [("2024-07-15", 5000.0)],  # консолидация 5000:1; цена x4640, объём /5158, оборот x0,91
    "PLZL":  [("2025-03-27", 0.1)],     # дробление 1:10; цена /10,6, объём x15,9, оборот x1,48
    "T":     [("2026-04-17", 0.1)],     # дробление 1:10; цена /9,99, объём x9,95, оборот x0,98
}
...
def breadth(tickers=None, start=None, end=None, bootstrap=False):
    ...
    for ticker in names:
        ...
        history.update(new)
        if history:
            # Поправку на дробления применяем ТОЛЬКО к счётной копии: в сторе
            # ряд px_<тикер> остаётся сырым, каким его отдал ISS. Иначе один
            # прогон переписал бы историю цен, и отличить «поправлено» от
            # «источник так отдал» стало бы нельзя уже никогда.
            merged[ticker] = _apply_corporate_actions(ticker, history)
        else:
    ...
    for ticker, points in fresh.items():
        ...
        out.append((sid, points, make_meta("iss", urls.get(ticker), points, unit="rub",
                                           secid=ticker,
                                           note="сырьё для breadth: СЫРЫЕ цены ISS, "
                                                "без поправки на дробления "
                                                "(поправка — _apply_corporate_actions)")))
    return out


def _apply_corporate_actions(ticker, points):
    """Привести цены бумаги к масштабу ПОСЛЕ всех её дроблений (CORPORATE_ACTIONS).

    События применяются по возрастанию даты, и множители накапливаются сами собой:
    точка старше обоих дроблений умножается на оба, точка между ними — только на
    второе. Бумага без событий возвращается как есть, без копии словаря.
    """
    actions = CORPORATE_ACTIONS.get(ticker)
    if not actions or not points:
        return points
    out = dict(points)
    for day, factor in sorted(actions):
        for stamp, value in list(out.items()):
            if stamp < day:
                out[stamp] = value * factor
    return out
