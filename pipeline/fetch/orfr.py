...
def _period_from_name(url):
    ...
    Пары с «месяцем» вне 1..12 отбрасываем: у ЦБ реально висит файл
    ORFR_2024-25-1.pdf (это диапазон лет, а не год-месяц), и он давал период
    '2024-25' — _month_end на нём падал ValueError «month must be in 1..12».
    Сейчас не стреляет только потому, что лексически '2026-02' > '2024-25';
    стоит ЦБ так назвать свежий выпуск — и flows() валится вместо честного
    manual_needed, то есть ручной фолбэк inputs/orfr.yml не отработает.
    """
    names = [(y, m) for y, m in re.findall(r"(\d{4})-(\d{1,2})", url.rsplit("/", 1)[-1])
             if 1 <= int(m) <= 12]
    if not names:
        return None
    year, month = names[-1]
