...
def _age_min(ts, now):
    """Сколько минут назад опрашивали. Отрицательного возраста не бывает.

    `now` берётся ОДИН раз в начале прогона (снимок должен быть внутренне
    ...
    начинаются повторы — три попытки на ряд, — и прогон растягивается до 73–77 секунд.
    Ряд, опрошенный на второй минуте, получает `fetched_at` ПОЗЖЕ, чем `now`, и
    возраст уходит в минус.

    Скверно здесь то, что промах случается не случайно, а ровно в тех прогонах, где
    источник сбоит, — то есть попадает именно в текст тревоги о нём. 8 сентября
    владелец получил «Последний удачный опрос -1 мин назад» и справедливо не понял,
    что это значит.

    Ноль читается верно («только что») и ничего не скрывает: решения принимаются по
    `status`, а не по этому числу, оно только для чтения человеком.
    """
