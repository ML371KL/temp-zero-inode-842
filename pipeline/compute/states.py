...
def _n(v, nd=1, plus=False):
    """Число с типографским минусом. Менять минус во всей строке нельзя: рядом живут
    дефисы «80-й перцентиль» и «252-дневный максимум» — они не минусы."""
...
def _distances(cols, current):
    ...
    if calc.is_num(rv) and calc.is_num(thr):
        ...
        out.append({
            ...
            "text": f"реализованная вола {rv * 100:.1f}% — на {abs(gap):.1f} п.п. "
                    f"{side} порога {thr * 100:.1f}% (80-й перцентиль за 3 года)",
        })
