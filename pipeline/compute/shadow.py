...
def _sig_rotation_trigger(ctx):
    ...
    # С 2027 года Минфин предлагает облагать доход внутри ПИФ 15% (бюджетный пакет
    # 24.09.2026): если закон примут, отток из фондов ликвидности сменит смысл —
    # деньги смогут уходить во вклады, и срабатывания до и после копить вместе нельзя.
    note = ("накапливаем: в исследовании 16 точек, срабатываний не было; переток из фондов "
            "ликвидности в акции — гипотеза. С 2027 года возможен налог 15% на доход внутри "
            "ПИФ: срабатывания до и после него — разные выборки.")
