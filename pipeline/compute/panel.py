...
FFILL_LIMITS = {
    ...
    # brent: 7 -> 10 (12.08.2026). Семёрка была взята из валидации, где ряд лежал
    # файлом и «лага публикации» не существовало. У живого источника он есть, и
    # он НЕДЕЛЬНЫЙ: FRED отдаёт DCOILBRENTEU из EIA, а EIA публикует эту «дневную»
    # серию раз в неделю — на странице серии стоит «Release Date: 8/5/2026, Next
    # Release Date: 8/12/2026». 12.08.2026 хвост дошёл ровно до семи торговых дней,
    # и один задержавшийся релиз обнулял бы rb_gap (активный сигнал тира B) и
    # brent_mom63 — без единой строки в журнале, потому что фетчер при этом «ok».
    # Десять — полтора недельных цикла, как у депозитных декад (цикл 7, лимит 15).
    # На истории не меняет НИЧЕГО: внутренних разрывов длиннее 5 календарных дней
    # в 9947 точках ряда нет (проверено), инвариант композита сохранён побитово.
    "hy": 3, "brent": 10, "futoi": 3, "breadth": 3,
}
...
# Протяжка ноги ядра urals_rub_gap. Месячная точка живёт до следующей (~22 торговых
# дня) плюс лаг публикации и опоздания релиза; 66 торговых дней ≈ три месяца — с
# запасом на самый поздний релиз, но конечно: мёртвый источник при «ok» от зеркала
# (SLA не срабатывает) раньше кормил треть композита КОНСТАНТОЙ бессрочно, и
# degraded=false скрывал замёрзшую ногу. У соседних ног лимит есть (deposit 15,
# brent 10) — у этой не было. Сохранность истории проверена побитово: инвариант
# композита 199/199 месяцев после введения лимита (аудит 18.08.2026).
URALS_FFILL_LIMIT = 66
...
def _urals_rub_gap(store, dates, usd):
    ...
    return _align(gap, dates, _lag_days("urals_tax"), URALS_FFILL_LIMIT)
