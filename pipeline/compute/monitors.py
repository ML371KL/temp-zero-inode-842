...
def _t_cb_meeting(store, now):
    ...
    cons = dict(cons_pts).get(nxt) if nxt else None
    if cons is None:
        _, cons = _last(cons_pts)
    ...
    if nxt is None:
        ...
        headline = (f"Заседание сегодня. Ключ {_n(key_rate, 2)}%, консенсус "
                    f"{_n(cons, 2)}% ({_n(delta_bp, 0, True) if delta_bp is not None else 'н/д'} б.п.)")
    else:
        headline = (f"До заседания {_ddmm(nxt)} — {days} дн. Ключ {_n(key_rate, 2)}%, "
                    f"консенсус {_n(cons, 2)}%")
    return _tile("cb_meeting", status, nxt or key_asof, headline, payload,
...
def _t_futoi(store, now):
    ...
    if z is None:
        verdict = "истории мало для z-120"
    elif z >= 1.0:
        verdict = "физики перегружены лонгом — контрариан против роста"
    elif z <= -1.0:
        verdict = "физики перегружены шортом — контрариан за рост"
    else:
        verdict = "позиционирование нейтральное"
    holders = ""
    if hl.get(asof) or hs.get(asof):
        holders = f"; держателей лонга {_n(hl.get(asof), 0)}, шорта {_n(hs.get(asof), 0)}"
    headline = f"Нетто физлиц z(120д) {_n(z, 2, True)}: {verdict}{holders}"
...
def _t_rvi(store, now):
    ...
    return _tile("rvi", status, asof, headline, payload,
                 "Уровень и «зоны» RVI не предсказывают ничего, пересечение 50 — артефакт 2022 года.",
                 meta.get("fetched_at"))
...
def _t_rub_barrel(store, now):
    ...
    usd_month = {d[:7]: v for d, v in usd_pts}
    usd_for_month = usd_month.get(u_asof[:7]) or usd_pts[-1][1]
    ...
    return _tile("rub_barrel", status, u_asof, headline, payload,
                 "Знак КОНТРАРИАН: высокая бочка к своему 2-летнему тренду → более низкие "
                 "форвардные доходности (IC −0,19). Прокси считается через дисконт Urals к Brent.",
                 u_meta.get("fetched_at"))


def _t_sep_node(store, now):
    """Календарный бейдж: узел 10.09–05.10. Источник — календарь, не данные,
    поэтому статус всегда ok (нечему протухать)."""
    today = _msk_now(now).date()
...
def _t_mcxsm(store, now):
    ...
    rs63 = _rs(63)
    side = "лучше" if (rs63 or 0) >= 0 else "хуже"
    headline = f"Малые каппы {side} индекса на {_n(abs(rs63) if rs63 is not None else None, 1)}% за 63 дня"
