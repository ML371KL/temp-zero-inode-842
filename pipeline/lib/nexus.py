...
# Граница предложения = точка (или ! ?), пробел за ней И ЗАГЛАВНАЯ БУКВА (либо
# цифра, кавычка, тире) дальше. Числа вида «+1.4%/мес» и «−0.55%» под правило не
# попадают: после десятичной точки пробела нет.
#
# Требование заглавной — не косметика, а единственное, что отличает конец фразы от
# сокращения. Русские тексты панели сплошь из сокращений с точкой: «б.п.», «п.п.»,
# «руб.», «млрд руб.», «нед. назад». Без этого условия заголовок в общей ленте хаба
# рвался посреди фразы: «ЦБ: ключевая 15.75% (−25 б.п.) — СЮРПРИЗ +25 б.п.» и
# подпись «к консенсусу 15.50%.» — читатель видит обрубок, причём именно в
# сообщении о решении ЦБ, то есть в самом важном.
#
# Цена ошибки в другую сторону мала: предложение, начатое со строчной, просто не
# разрежется, и хаб покажет весь текст заголовком — так же, как до появления
# разреза вообще.
