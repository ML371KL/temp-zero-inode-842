...
# ------------------------------------------------------- DNS платного шлюза
# Резолвер Hetzner (185.12.64.x) отвечает SERVFAIL на apim.moex.com: авторитетные
# NS зоны (nic.ru) не отвечают из этой сети, хотя публичные резолверы отдают
# адрес, а TCP/TLS до него живы (замер 01.09.2026; началось 27.08.2026). Без
# записи futoi молча деградирует до бесплатного потока, а тот отказывает в
# последних 14 днях. Закрепляем адрес точечно — hosts-файлом, не переключая
# резолвер всей машины: на ней живут и другие панели.
# Если MOEX сменит IP: запросы начнут падать громко (timeout/TLS), meta futoi
# скажет «платный шлюз недоступен» — тогда обновить IP здесь и в /etc/hosts
# (актуальный смотреть: dig +short apim.moex.com @1.1.1.1).
say "DNS платного шлюза ALGOPACK"
if getent hosts apim.moex.com >/dev/null 2>&1; then
  note "apim.moex.com резолвится — запись в hosts не нужна"
elif grep -q "apim.moex.com" /etc/hosts; then
  note "запись в /etc/hosts уже есть"
else
  echo "85.118.181.43 apim.moex.com  # ALGOPACK: резолвер хостера SERVFAILит, закреплено установщиком repo-842" >> /etc/hosts
  note "apim.moex.com закреплён в /etc/hosts (85.118.181.43)"
fi
